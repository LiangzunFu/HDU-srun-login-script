...
class LoginManager:
    ...
    def _resolve_login_responce(self):
        print("===== 登录响应原文（raw response）=====")
        print(self._login_responce.text)
        print("===================================")

        # 清理回调包装（去掉 jQuery...(...)）
        json_text_match = re.search(r'\((\{.*\})\)', self._login_responce.text)
        if not json_text_match:
            raise RuntimeError("响应格式错误，无法提取 JSON")

        json_text = json_text_match.group(1)
        res_dict = json.loads(json_text)

        if res_dict.get("res") == "ok":
            self._login_result = "登录成功"
        elif "error_msg" in res_dict:
            self._login_result = f"登录失败：{res_dict['error_msg']}"
        else:
            self._login_result = "未知登录状态"
